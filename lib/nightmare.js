...
function Nightmare(options) {
  ...
  var runnerOptions = {};
  if(options.paths) {
    runnerOptions.paths = options.paths;
  }
  if(options.chromiumArgs) {
    runnerOptions.chromiumArgs = options.chromiumArgs;
  }
  if (runnerOptions)
    runnerArgs.push(JSON.stringify(runnerOptions));

  console.log(runnerArgs);
  this.proc = proc.spawn(electron_path, runnerArgs, {
