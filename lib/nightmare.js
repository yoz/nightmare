...
var debugElectron = require('debug')('nightmare:electron');
...
function Nightmare(options) {
  ...
  var runnerArgs = ['--v=1', '--enable-logging', runner];
  this.proc = proc.spawn(electron_path, runnerArgs.concat(electronArgs), {
    stdio: [null, null, null, 'ipc']
  });
  this.proc.stdout.on('data', function (data) {
    debugElectron('stdout: ' + data);
  });
  this.proc.stderr.on('data', function (data) {
    debugElectron('stderr: ' + data);
  });
  this.proc.on('close', function (code) {
    debugElectron('child process exited with code ' + code);
  });
