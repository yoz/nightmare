...
if (process.argv.length > 2) {
  var options = JSON.parse(process.argv[4]);
  console.log(options);
  if (options.paths) {
    for (var i in options.paths) {
      app.setPath(i, options.paths[i]);
    }
  }
  if (options.chromiumArgs) {
    for (var i in options.chromiumArgs) {
      if (options.chromiumArgs[i] === true) {
        app.commandLine.appendSwitch(i);
      } else {
        app.commandLine.appendSwitch(i, options.chromiumArgs[i]);
      }
    }
  }
}
